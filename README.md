...
4. 一般的な評価値の算出を含む、実験テンプレートとしてそのほかのタスクにも簡単に利用できる汎用的な実装
...
さらに、本実装ではJupyter NotebookなどNotebook形式のファイルを一切利用していません。
これは経験上、Notebook形式のファイルのデバッグが極めて困難であり、バグの温床になることが多いためです。
...
しかし、`conda`コマンドでインストールできるパッケージはしばしば古く、管理が難しいです。
...
これは、PyTorchのインストールには非常に多くの複雑怪奇な依存関係が存在する(例えば、システムのGCCのバージョンなど)ため、ここで発生する問題をできるだけ避けるためです。
...
以降はPoetryを用いて環境構築を想定した場合のコマンドを紹介しますが、minicondaを用いて環境構築をした場合は、`poetry run python`と書いてある部分を`python`に読み替えてください。
...
JSONL形式は近年よく利用されるようになってきたデータ形式であり、素直にPythonで読み込むことが可能である点、csv/tsvと比較して配列や階層のあるオブジェクトの表現が自然な点から、機械学習用途で用いるには極めて扱いやすいデータ形式です。
...
特筆すべき点として、本実装では学習後のモデルは保存せず、訓練のたびに評価値を算出し、評価値のみを保存するようにしています。
